...
/**
 * Abstract implementation of the InterruptibleRunnable including the
 * cancel/pause functionality. What's left in the subclasses is to implement the
 * {@link #execute()} method with your functionality and to check the
 * interruption from time to time by simply calling {@link #checkInterruption()}
 * . <br/>
 * Please make sure not to surround the code calling
 * {@link #checkInterruption()} with a try/catch on {@link Exception}, as this
 * will block the interruption in case of a CANCEL call. The
 * {@link #checkInterruption()} is actually throwing a {@link CancelException}
 * that should be caught in {@link #run()}, which is the trick used to cancel
 * the execution. <br/>
 * In case of pause, simply calling {@link #pause()} will block the execution of
 * the thread at the next {@link #checkInterruption()} call. A pause can be call
 * off with a {@link #resume()} call.
 * 
...
public abstract class AbstractInterruptibleRunnable implements InterruptibleRunnable {
    ...
    public final void run() {
        ...
        //
        // We keep the NOT_RUNNING state just to make the difference between the
        // CANCEL and the
        // initial state of the process. It's a bit weird to say that the
        // initial state before running,
        // is CANCELED, while the process never run.
        synchronized (this) {

            // FIXME Make it more clear that when a process gets CANCELED, in
            // order to start it again,
        ...
        try {
            // Just to make sure if the process gets canceled immediately after
            // it's started.
            checkInterruption();
            ...
            // Finally set the state back to NOT_RUNNING, only and only if the
            // current state is RUNNING.
            // In case of a CANCEL state we want to keep the CANCEL state as
            // current, to let the user know that the
    ...
    /**
     * Write here any piece of code your process must execute, that normally
     * you'd write in the {@link #run()} method. <br/>
     * Also make sure to call constantly {@link #checkInterruption()} to stop or
     * pause your process when this is required. And make sure that at any
     * level, these calls are not surrounded with a try/catch on
     * {@link Exception}, otherwise the cancel operation won't work.
     * 
     * @throws Exception
     ...
     * <br/>
     * We don't need this method abstract because some times you don't need to
     * do any destruction at the end of the process.
     */
    protected void executed() {
        // Override this and perform any destruction. But do it fast, you don't
        // need to keep this call too long.
        //
        // Any exception handling should be done by the implementor of this
        // method.
    }
    ...
    protected void checkInterruption() {
        ...
        if (state == PAUSED) {
            synchronized (this) {
                ...
                // Double check after lock to make sure we're not messing
                // notify() with wait().
                if (state == PAUSED) {
                    try {
                        setRealState(PAUSED);
        ...
        // Don't do it with an else because if cancel after pause, this should
        // throw the exception here and cancel the process.
        if (state == CANCELED) {
            synchronized (this) {
    ...
    /*
     * The real state, according with the exact time the process actually gets
     * paused or canceled.
     */
